"""
LSSIREN (Large-Scale Siren) Dataset Dataloader
===============================================
PyTorch Dataset and Lightning DataModule for Large-Scale Emergency Vehicle Sirens and Road Noises.
...
def load_label_mapping(dataset_name: str = "LSSIREN", 
...
class LSSirenDataset(Dataset):
    """
    Large-Scale Siren Dataset for audio classification.
    ...
    def __init__(self,
        ...
        """
        Initialize LSSiren Dataset.
        
        Args:
            folder_path: Path to root folder containing CSV files and audio folders
        ...
        # Load label mapping
        self.label_map = label_map if label_map is not None else load_label_mapping("LSSIREN")
    ...
    def _load_files(self) -> Tuple[List[str], List[int]]:
        ...
        # Mapping from label_map keys to (csv_file, audio_folder)
        category_info = {"Ambulance_data": ("Ambulance_final.csv", "Ambulance_data"),
                         "Road_Noises": ("Road_final.csv", "Road_Noises")}
        
        for category, label in self.label_map.items():
            if category not in category_info:
                print(f"Warning: Unknown category '{category}' in label_map")
                continue
            
            csv_file, audio_folder = category_info[category]
            csv_path = os.path.join(self.folder_path, csv_file)
            audio_folder_path = os.path.join(self.folder_path, audio_folder)
            ...
            if not os.path.exists(audio_folder_path):
                print(f"Warning: Audio folder not found: {audio_folder_path}")
                continue
            ...
            try:
                ...
                # Get filenames from CSV - use .iloc or direct column access
                if 'filename' in df.columns:
                    filenames_list = df['filename'].tolist()
                else:
                    # Try first column
                    filenames_list = df.iloc[:, 0].tolist()
                
                for filename in filenames_list:
                    # Clean filename (remove whitespace)
...
class LSSirenDataModule(pl.LightningDataModule):
    """
    PyTorch Lightning DataModule for Large-Scale Siren dataset.
    ...
    def __init__(self,
        ...
        Args:
            folder_path: Path to root folder with CSV files and audio folders
    ...
    def _setup_train_mode(self):
        ...
        # Create full dataset
        full_dataset = LSSirenDataset(folder_path=self.folder_path,
    ...
    def _setup_benchmark_mode(self):
        """Setup for benchmark mode: use all data as test set."""
        self.test_dataset = LSSirenDataset(folder_path=self.folder_path,
                                           label_map=self.label_map,
                                           seed=self.seed,
...
if __name__ == "__main__":
    ...
    print("=" * 80)
    print("DATALOADER TEST - LSSiren (Large-Scale Siren)")
    print("=" * 80)
    ...
    dm_train = LSSirenDataModule(folder_path=current_dir,
                                 mode='train',
                                 seed=42,
                                 batch_size=32,
    ...
    dm_bench = LSSirenDataModule(folder_path=current_dir,
                                 mode='benchmark',
                                 seed=42,
                                 batch_size=32,
                                 target_sr=32000,
