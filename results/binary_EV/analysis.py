"""
Binary Emergency Vehicle Classification - Training Analysis
=============================================================
...
def load_test_metrics(model_dir: Path) -> Dict:
    """Load test metrics including confusion matrix."""
...
def plot_validation_metrics_fig1(validation_data: Dict):
    ...
    output_path = FIGURES_DIR / 'binary_EV_validation_accuracy_f1score.svg'
...
def plot_validation_metrics_fig2(validation_data: Dict):
    ...
    # Save figure
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    output_path = FIGURES_DIR / 'binary_EV_validation_precision_recall.svg'
    plt.savefig(output_path, dpi=600, format='svg', bbox_inches='tight')
    print(f"✓ Saved: {output_path}")
    plt.close()
...
def plot_confusion_matrices(test_data: Dict):
    """Figure 3: Confusion matrices for all 6 models (2 rows x 3 columns)."""
    ...
    # Save figure
    FIGURES_DIR.mkdir(parents=True, exist_ok=True)
    output_path = FIGURES_DIR / 'binary_EV_confusion_matrices.svg'
    plt.savefig(output_path, dpi=600, format='svg', bbox_inches='tight')
    print(f"✓ Saved: {output_path}")
    plt.close()
...
def main():
    ...
    print("\n" + "="*70)
    print("Binary EV Classification - Training Analysis")
    print("="*70 + "\n")
